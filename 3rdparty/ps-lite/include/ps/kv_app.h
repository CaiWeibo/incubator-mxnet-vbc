...
public:
    ...
    int ZPush(
              ...
              int uniq_key = 0,
              int version = 0) {
        PS_VLOG(1) << "Push key: " << uniq_key << "to " << keys.size() << "servers in version: " << version;
    ...
    void AutoPullUpdate(const int version, const KVMeta& req, const KVPairs<Val>& kvs = KVPairs<Val>()) {
        ...
        for (auto receiver : Postoffice::Get()->GetNodeIDs(kWorkerGroup)){
            if(kvs.keys.size()){
                ...
                PS_VLOG(2) << "Send a auto to key: " << req.key;
        ...
        //obj_->AddResponse(ts, num); //无需接收来自worker的应答
        PS_VLOG(1) << "Send an auto pull for key: " << req.key << "to all workers.";
    }
...
void KVServer<Val>::Process(const Message& msg) {
    ...
    PS_VLOG(1) << "Get a package for key: " << meta.key << "from worker: " << meta.sender << "with push=" << meta.push << " in version: " << meta.version;
    KVPairs<Val> data;
    int n = msg.data.size();
    if (n) {
        CHECK_GE(n, 2);
...
void KVServer<Val>::Response(const KVMeta& req, const KVPairs<Val>& res) {
    ...
    Postoffice::Get()->van()->Send(msg);
    PS_VLOG(1) << "Send a response for key: " << req.key << " to worker: " << req.sender;
}
...
void KVWorker<Val>::DefaultSlicer(
    ...
    CHECK_EQ(pos[n], send.keys.size());//末位位置为keys的数量
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (send.lens.size()) {
            ...
            for (int l : kv.lens) val_end += l;//变量末端加长度
...
void KVWorker<Val>::Process(const Message& msg) {
    ...
    if (msg.data.size()) {
        ...
        if (msg.meta.request){//此消息为server的自动更新
            PS_VLOG(1) << "Get an auto pull for key: " << key << " from server: " << msg.meta.sender;
            ...
            PS_VLOG(1) << "Get a pull response for key: " << key << " from server: " << msg.meta.sender;
            mu_.lock();
            recv_kvs_[ts].push_back(kvs);
            mu_.unlock();
        }
    }else{
        PS_VLOG(1) << "Get a push response for key: " << key << " from server: " << msg.meta.sender;
    }
...
int KVWorker<Val>::AutoPull(int uniq_key, const SArray <Key> &keys, SArray <Val> *vals, SArray<int> *lens, int cmd,
    ...
    PS_VLOG(1) << "A new auto pull: " << uniq_key;
    //std::vector<KVPairs<Val>> autokvs;
    std::unique_lock<std::mutex> lk(mu_);
    while(auto_pull_kvs_[uniq_key].size() != keys.size()){
        PS_VLOG(1) << uniq_key << "gets " << auto_pull_kvs_[uniq_key].size() << " needs: " <<keys.size();
        cond_.wait(lk);
    }
    PS_VLOG(1) << "Gets all keys"<< uniq_key ;
    auto& autokvs=auto_pull_kvs_[uniq_key];
    /*
    for(auto& kvs : auto_pull_kvs_[uniq_key]){
        autokvs.push_back(kvs.second);
    }*/
    ...
    //do sort
    /*
    std::sort(autokvs.begin(), autokvs.end(), [](
            const KVPairs<Val>& a, const KVPairs<Val>& b) {
        return a.keys.front() < b.keys.front();
    });*/
    ...
    for (int i=0; i<keys.size(); i++){
        ...
        //memcpy(p_vals, kvs.vals.data(), kvs.vals.size() * sizeof(Val));
        //p_vals += kvs.vals.size();
        if (p_lens) {
    ...
    auto_pull_kvs_.erase(uniq_key);
    //lk.unlock();
    //PS_VLOG(2) << "Delete used key" << uniq_key;
